...
const numberOfPages = database.length;
const renderPages = database.map(
  (page, index) =>
    `
      <span name='page'>
        ${index + 1}
...
pages.innerHTML = renderPages.join('');

const body = document.querySelector('.bodyContainer');

function renderCards(page) {
  return database[page].map(
    (project) =>
      `
      <div class='card'>
        <img
...
const allPages = document.getElementsByName('page');
if (allPages) {
  for (let i = 0; i < allPages.length; i++) {
    allPages.item(i).addEventListener('click', () => {
      selectedPage = allPages.item(i).innerText;
      body.innerHTML = renderCards(selectedPage - 1).join('');
      window.localStorage.setItem('selectedPage', Number(selectedPage))

      const links = document.getElementsByTagName('h1');

      if (links) {
        for (let i = 0; i < links.length; i++) {
          links.item(i).addEventListener('click', () => {
            window.localStorage.setItem('selectedLink', links.item(i).innerText);
...
const cards = renderCards(selectedPage - 1)
